...
def clear_zombie_windows(user_obj):
    ...
    for window in windows:
        ...
        if window_num not in valid_windows:
            print(
                f"{INFO} clear_zombie_windows: Killing window {user_obj.tmux_name}:{window_num}"
            )
...
def solve_env(tpu):
    ...
    if tpu_status == "preempted":
        ...
        if res == "success":
            ...
            print(
                f"{FAIL} solve_env: Reapply TPU {tpu} failed, please contact the admin"
            )
    ...
    if state == "success":
        ...
        print(
            f"{INFO} solve_env: Environment in TPU has file error, trying to mount disk..."
        )
        res = mount_disk(tpu)
        if res == "success":
            ...
            print(
                f"{FAIL} solve_env: Solving environment in TPU {tpu} failed, please contact the admin"
            )
            return "failed"
    elif state == "unknown":
        print(
            f"{FAIL} solve_env: Environment in TPU {tpu} is unknown error, please contact the admin"
        )
        return "failed"
...
def initialization():
    ...
    for user in data["users"]:
        ...
        print(
            f"{INFO} initialization: Killing tmux session {data['users'][user]['tmux_name']}"
        )
        os.system(f'tmux kill-session -t {data["users"][user]["tmux_name"]}')
        time.sleep(5)
        print(
            f"{INFO} initialization: Recreating tmux session {data['users'][user]['tmux_name']}"
        )
...
def change_ip():
    ...
    try:
        for user in data["users"]:
            ...
            # time.sleep(5)
            print(
                f"{INFO} change-ip: Recreating tmux session {data['users'][user]['tmux_name']}"
            )
            os.system(f'tmux new-session -d -s {data["users"][user]["tmux_name"]}')
            print(
                f"{GOOD} change-ip: Recreating tmux session {data['users'][user]['tmux_name']} done"
            )
    except Exception as e:
    ...
    for user in data["users"]:
        for job in data["users"][user]["job_data"]:
            if job["status"] == "running":
                os.system(
                    f'tmux new-window -t {data["users"][user]["tmux_name"]}:{job["windows_id"]}'
                )
    ...
    for user in data["users"]:
        for job in data["users"][user]["job_data"]:
            if job["status"] == "running":
                ...
                if res == "y":
                    print(
                        f"{INFO} change-ip: Resuming job in windows {job['windows_id']} for user {user}"
                    )
                    resume_rerun_job(job, load_ckpt=True)
                else:
                    print(
                        f"{INFO} change-ip: Not resuming job in windows {job['windows_id']} for user {user}"
                    )
